...
namespace GetDataTools.PoDu
{
    ...
    public class SlopeDem
    {
        ...
        private static Dictionary<int, OrderPoint> _myRelaDic;

        public static SlopeDem SlopeDemObj
        {
            get
            {
                if (_slopeDemObj == null)
                {
                    _slopeDemObj = new SlopeDem();
                    InitDictionary();
                }
                return _slopeDemObj;
            }
        ...
        public void CreatePolygon(string inDataPath ,string OutShpPath)
        {
            ...
            GeneralShp gShp = new GeneralShp(OutShpPath, wkbGeometryType.wkbPolygon);
            ...
            for (int _j = 0; _j < _outDataset.RasterYSize; _j++)
            {
                for (int _i = 0; _i < _outDataset.RasterXSize; _i++)
                {
                    if (IsImportPnt(_outDataset, _i, _j, _importLevel) && !_beVisited[_i, _j])
                    {
                        ...
                        if (_orStartPnt != null && _orSecondPnt != null)
                        {
                            ...
                            if (_disOf2Pnt < Math.Sqrt(51) && (_resPntList.Count + _resPntList_unClock.Count) > 8)
                            {
                                /****************************************/
                                //新建一个polygon对象
                                ...
                                gShp.InsertFeature(ring);
                                /****************************************/
                            }
                        }
                    }
                    //将改点设置为已被访问过
                    _beVisited[_i, _j] = true;
                }
            }
            gShp.Close();
            _outDataset.Dispose();
